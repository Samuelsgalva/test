 ...
 * - El payload es la conversación directamente: { id, meta: { sender, assignee, channel }, messages, ... }
 *
 * Uso:
 *   <script src="chatwoot-context.js"></script>
 *   <script>
 *     const ctx = new ChatwootContext({ debug: true, debugPanelId: 'debugPanel' });
...
class CWConversation {
  ...
  /** Datos raw del sender dentro de meta */
  get senderRaw()   { return this.#raw.meta?.sender ?? {}; }

  /** Datos raw del assignee dentro de meta */
  get assigneeRaw() { return this.#raw.meta?.assignee ?? {}; }

  /** Copia completa del payload original */
...
class ChatwootContext {
   ...
   */
  constructor({ debug = false, debugPanelId = null, timeoutMs = 5000 } = {}) {
  ...
  // ── Eventos ────────────────────────────────
  // Disponibles: 'contextReady' | 'contextUpdated' | 'contextTimeout' | 'rawMessage'
  ...
  init() {
    ...
    // Solicitar contexto según la documentación oficial
    this.fetchContext();
  ...
  #handleMessage(event) {
    ...
    // Chatwoot envía JSON string — parseamos
    let parsed = null;
    ...
    if (typeof event.data === 'string') {
      try {
        ...
        // No es JSON, ignorar (puede ser un postMessage de otra fuente)
        return;
      }
    ...
    // Validar que tenga datos reconocibles de Chatwoot
    // El payload principal es la conversación directa: { id, meta: { sender, assignee, channel }, ... }
  ...
  #looksLikeChatwootPayload(data) {
    // El formato documentado tiene id + meta.sender
    if (data.id && data.meta?.sender) return true;
    // Fallback: si viene envuelto de alguna forma
  ...
  #applyContext(data) {
    ...
    // Formato principal documentado: la data ES la conversación
    // { id, status, meta: { sender, assignee, channel }, messages, ... }
